  ...
  namespace1: <node-selectors-labels>
	...
	Describe("#ValidateWorker", func() {
		...
		Describe("#ValidateCloudProfileReference", func() {
			...
			It("should allow switching from a NamespacedCloudProfile to a CloudProfile", func() {
				oldCloudProfileReference := &core.CloudProfileReference{
					Kind: "NamespacedCloudProfile",
					Name: "my-profile",
				}
			...
			It("should not allow switching the name of the CloudProfile coming from a cloudProfileName", func() {
				...
				Expect(errList).To(ConsistOf(
					PointTo(MatchFields(IgnoreExtras, Fields{
						"Type":   Equal(field.ErrorTypeForbidden),
						"Field":  Equal("cloudProfile"),
						"Detail": Equal("the cloud profile reference can only be changed from a CloudProfile to a directly descendant NamespacedCloudProfile and back"),
					}))))
			})
			...
			It("should not allow switching the name of the NamespacedCloudProfile coming from a cloudProfile", func() {
				oldCloudProfileReference := &core.CloudProfileReference{
					Kind: "NamespacedCloudProfile",
					Name: "my-profile",
				}
