...
var _ = Describe("Shoot Tests", Label("Shoot", "default"), func() {
	test := func(shoot *gardencorev1beta1.Shoot) {
		...
		Describe("Create, Update, Delete", Label("simple"), Offset(1), Ordered, func() {
			var (
				ctx    context.Context
			...
			It("Verify shoot access using admin kubeconfig", func() {
			...
			It("Verify shoot access using viewer kubeconfig", func() {
				Eventually(func(g Gomega) {
			...
			It("Verify worker node labels", func() {
				if !v1beta1helper.IsWorkerless(f.Shoot) {
			...
			It("Add skip readiness annotation", func() {
			...
			It("Wait for operation annotation to be gone (meaning controller picked up reconciliation request)", func() {
				Eventually(func(g Gomega) {
