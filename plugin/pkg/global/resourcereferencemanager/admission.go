...
func (r *ReferenceManager) ensureShootReferences(ctx context.Context, attributes admission.Attributes, oldShoot, shoot *core.Shoot) error {
	...
	if !equality.Semantic.DeepEqual(newCloudProfileReference, oldCloudProfileReference) {
		...
		if oldCloudProfileReference != nil &&
			oldCloudProfileReference.Kind == v1beta1constants.CloudProfileReferenceKindNamespacedCloudProfile &&
			newCloudProfileReference.Kind == v1beta1constants.CloudProfileReferenceKindCloudProfile {
			...
			for _, w := range shoot.Spec.Provider.Workers {
				...
				if _, exists := machineTypes[w.Machine.Type]; !exists {
					return fmt.Errorf("newly referenced cloud profile does not contain the machine type %q currently in use by worker \"%s\"", w.Machine.Type, w.Name)
				}

				if w.Volume != nil && w.Volume.Type != nil {
					if _, exists := volumeTypes[*w.Volume.Type]; !exists {
						return fmt.Errorf("newly referenced cloud profile does not contain the volume type %q currently in use by worker \"%s\"", *w.Volume.Type, w.Name)
					}
				}
			}
		}
